...
let qrScanner=null;
let audioCtx=null;
const STORAGE_KEY="hotel-pro-v15-final";
...
const label=$("#navRole");if(label) label.textContent=r==="hk"?"PETUGAS • v15 FINAL":"TAMU • SCAN QR";
renderAll();updateViewers();updateInsights();
if(r==="guest"){startNeedStreamLoop();}else{stopNeedStreamLoop();}
if(peer){destroyPeer();}
}
function renderFlow(){
const steps=[
{title:"Tamu Datang",desc:"Check-in",tag:"Datang",icon:"🛬"},
{title:"Cek Kamar",desc:"Sistem cek",tag:"Cek",icon:"💻"},
{title:"Live Status",desc:"Tamu lihat HP",tag:"Live",icon:"📱"},
{title:"Bersihkan",desc:"Checklist + kamera",tag:"Bersih",icon:"🧹"},
{title:"Progres",desc:"Foto bukti",tag:"Progress",icon:"📊"},
{title:"Kamar Siap",desc:"Animasi + suara",tag:"Siap",icon:"🔔"},
{title:"Tamu Masuk",desc:"Terima kunci",tag:"Masuk",icon:"🚪"}
];
...
const statProg=$("#statProgress");if(statProg) statProg.textContent=p+"%";
};
...
const helpEta=$("#helpEta");if(helpEta) helpEta.textContent=p>=100?"Selesai! Siap dihuni":p>0?`~ ${Math.max(1,Math.round((100-p)/15))} menit lagi`:"Menunggu petugas";
}
...
if(appState.progress===100){triggerCelebration();}else{pushLog(`Progres ${appState.progress}%`);}
renderAll();updateInsights();saveLocal();
});
cl.appendChild(row);
}
if(gl){
...
e.stopPropagation();
if(role!=="hk"){alert("Hanya petugas");return;}
appState.photos.splice(i,1);renderPhotos();updateInsights();saveLocal();
});
d.append(img,del);strip.appendChild(d);
});
const countEl=$("#helpPhotoCount");if(countEl) countEl.textContent=`${appState.photos.length} foto bukti`;
const statPhoto=$("#statPhotos");if(statPhoto) statPhoto.textContent=`${appState.photos.length}`;
}
...
cs.innerHTML=live?'<span class="on-dot"></span> Live • Fullscreen':'<span></span> Offline';
...
function renderAll(){renderFlow();renderProgress();renderChecklist();renderPhotos();renderCamera();}
function updateQr(){
...
const vn=$("#viewerNames");if(vn) vn.textContent=count===0?"Belum ada penonton":`${count} perangkat • OK`;
...
const statV=$("#statViewers");if(statV) statV.textContent=`${count}`;
}
function updateInsights(){
const durEl=$("#statDuration");
if(durEl&&sessionStart){
const diff=Math.floor((Date.now()-sessionStart)/1000);
const m=String(Math.floor(diff/60)).padStart(2,"0");
const s=String(diff%60).padStart(2,"0");
durEl.textContent=`${m}:${s}`;
}else if(durEl&&!sessionStart){durEl.textContent="00:00";}
}
...
peer.on("close",()=>{stopHeartbeat();sessionStart=null;});
peer.on("error",err=>{
const cb=$("#connBox");if(cb) cb.className="cs err";
const cbG=$("#connBoxG");if(cbG) cbG.className="cs err";
...
conn.on("close",()=>{dataConns.delete(pid);mediaConns.delete(pid);updateViewers();updateInsights();});
}
...
renderPhotos();updateInsights();saveLocal();
...
const roomEl=$("#celeRoom");if(roomEl) roomEl.textContent=`KAMAR ${roomCode} selesai 100% — siap dihuni`;
if(cele) cele.classList.add("show");
playSuccessSound();
startConfetti();
if(navigator.vibrate) navigator.vibrate([120,60,120,60,200]);
pushLog(`🎉 Kamar ${roomCode} SELESAI 100%`);
...
},()=>{}).catch(()=>{alert("Gagal scanner");modal.classList.remove("show");});
});
if(closeBtn){
...
updateQr();
if(!appState.log.length){appState.log=[{t:new Date().toLocaleTimeString("id-ID",{hour:"2-digit",minute:"2-digit"}),msg:"Sistem siap • v15 • Fullscreen fix"}];saveLocal();}
...
$("#fixBlackBtn").addEventListener("click",()=>{startCameraFallback();});
$("#fixBlackGuestBtn").addEventListener("click",()=>{sendNeedStream();startNeedStreamLoop();});
...
const shareBtn=$("#shareLinkBtn");if(shareBtn) shareBtn.addEventListener("click",()=>{
const link=`${location.origin}${location.pathname}?room=${roomCode}&pin=${pinCode}`;
if(navigator.clipboard){navigator.clipboard.writeText(link).then(()=>{alert("Link disalin!");});}else{prompt("Copy link:",link);}
});
const dlBtn=$("#downloadAllBtn");if(dlBtn) dlBtn.addEventListener("click",()=>{
if(!appState.photos.length){alert("Belum ada foto");return;}
appState.photos.forEach((src,i)=>{
const a=document.createElement("a");a.href=src;a.download=`bukti-${roomCode}-${i+1}.jpg`;a.click();
});
});
const contactBtn=$("#contactBtn");if(contactBtn) contactBtn.addEventListener("click",()=>{const btn=$("#guestTalkBtn");if(btn) btn.scrollIntoView({behavior:"smooth"});});
const refreshHelp=$("#refreshHelpBtn");if(refreshHelp) refreshHelp.addEventListener("click",()=>{sendNeedStream();startNeedStreamLoop();});
setInterval(()=>{const c=$("#clock");if(c) c.textContent=new Date().toLocaleTimeString("id-ID",{hour:"2-digit",minute:"2-digit"});},1000);
setInterval(()=>{updateViewers();updateInsights();},1000);
setupTalkButtons();
initQrScanner();
initFullscreen();
setRole("hk");
